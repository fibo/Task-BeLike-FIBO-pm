
=head1 NAME

Task::BeLike::FIBO -- Leonardo Pisano a.k.a. Fibonacci

=head1 SYNOPSIS

    cpan Task::BeLike::FIBO
    perldoc Task::BeLike::FIBO

=head1 PACKAGE GUIDELINES

Do not get crazy with automatic generators. I am a mathematician and a coder, not a corporation.
Every package is different and has different needings. The followings are samples for files I usually need in a package.
Just use copy and paste and your brain!

The smack a L<DRY | http://en.wikipedia.org/wiki/Don't_repeat_yourself> L<KISS | http://en.wikipedia.org/wiki/KISS_principle> is not that bad.

=head2 WORKFLOW

=over 4

=item *

Run tests

    prove -l --state=save,failed

=item *

Create package

    perl Makefile.PL
    make
    make test
    make manifest
    make dist
    make realclean

=item *

Upload to PAUSE

Do it by hand. Yes, using your browser ... and remember to delete old files.

=back

=head2 FILES

=over 4

=item *

README.md

    My-Package
    ==========

    [![Build Status](https://travis-ci.org/fibo/My-Package-pm.svg)](https://travis-ci.org/fibo/My/Package-pm)

    See [online docs](https://metacpan.org/pod/My::Package)

=item *

.travis.yml

    language: perl
    perl:
      - "5.18"
      - "5.16"
      - "5.14"
      - "5.12"
      - "5.10"
      - "5.8"

=item *

.gitignore

    blib/
    Makefile
    Makefile.old
    MANIFEST.bak
    META.yml
    MYMETA.yml
    pm_to_blib
    .prove
    *.tar.gz

=item *

Makefile.PL

    use strict;
    use warnings;

    use ExtUtils::MakeMaker 6.64;

    my $name = 'My-Package';

    WriteMakefile(
        ABSTRACT_FROM => 'lib/My/Package.pm',
        VERSION_FROM  => 'lib/My/Package.pm',
        AUTHOR        => 'G. Casati <fibo@cpan.org>',
        NAME          => 'My-Package',
        META_MERGE   => {
          resources => {
            license     => 'http://fibo.mit-license.org/',
            repository  => 'http://github.com/fibo/' . $name . '-pm',
            bugtracker  => 'http://github.com/fibo/' . $name . '-pm/issues',
        },
        PREREQ_PM     => {
            # 'Some::Package' => '0',
            # 'Other::Package' => '1.2.3',
        },
        test          => { TESTS => 't/*.t' },
        TEST_REQUIRES => {
            'Test::More' => '1',
            'Test::Pod'  => '1',
        },
    );

=item *

MANIFEST.SKIP

    ^\.(?!prove)
    .*\.old$
    \.tar\.gz$
    ^Makefile$
    ^MYMETA\.
    ^blib
    ^pm_to_blib

=back

=head1 STUFF INCLUDED

=over 4

=item *

L<CPAN>

See how to setup L<A CPAN client that works like a charm|http://blog.g14n.info/2014/03/a-cpan-client-that-works-like-charm.html>.

=item *

L<ExtUtils::MakeMaker> version gt 6.96

=item *

L<Perl::Tidy>

Use L<Perl::Tidy> defaults to indent occasionally. Do not indent every source file automatically, indent by hand and use your creativity.

See Perl section in L<My Vim preferences|http://blog.g14n.info/2013/07/my-vim-configuration.html#perl> to see how you can use L<Perl::Tidy> with Vim.

=item *

L<Test::Compile>

=item *

L<Test::More>

=item *

L<Test::Pod>

=back

=cut

